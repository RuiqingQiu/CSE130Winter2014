...
let filename_to_expr f = 
  NanoParse.prog NanoLex.token (Lexing.from_channel (open_in f))

let string_to_expr s =
  NanoParse.prog NanoLex.token (Lexing.from_string s)

let execute f = 
  try Nano.strValue (Nano.eval ([],(filename_to_expr f))) 
  with exn -> "Error: "^(Printexc.to_string exn)

let execute_string s = 
  try Nano.strValue (Nano.eval ([],(string_to_expr s)))
  with exn -> "Error: "^(Printexc.to_string exn)

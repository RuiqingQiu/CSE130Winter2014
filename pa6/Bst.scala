...
sealed abstract class BST[A <% Ordered[A]] extends AbstractSet[A] 
{
  ...
  /**
   * Function: contains
   * Description: returns true iff the element x is in the tree.
   **/
  def contains(x: A): Boolean = {
    this match{
      //If it reaches leaf, then it's not in the path
      case Leaf() => false
      //Check the current element is the right element
      case Node(e, l, r) => if(e == x) true
                            //If not, then recursive search left and right
  ...
  /**
   * Function: fold
   * Description: a fold method that performs an in-order traversal of
   * the tree, accumulatng the results as it goes.
   **/
  ...
  /**
   * Function: add
   * Description: returns a new BST which has the same elements as 
   * original tree plus the new element x. 
   **/
  def add(x:A): BST[A] = { 
    this match{
      case Leaf() => Node(x, Leaf(), Leaf())
      case Node(e, l, r) => 
        //If it's less than, insert in left subtree
        if(x < e){
          Node(e, l.add(x), r)
        }
        else if (x > e){
        ...
        //If it's duplicate, then nothing is added
        else{
          this
        }
    }
  ...
  /**
   * Function: removeMin
   * Description: returns a tuple ofthe minimum element in the tree
   * and the tree containing all the elements except the minimum.
   **/
  def removeMin : (A, BST[A]) = {
    this match{
      case Leaf() => sys.error("The tree is empty")
      case Node(e, Leaf(), r) => (e, r)
      case Node(e, l, r) => 
        val (m, l_prime) = l.removeMin 
        (m, Node(e, l_prime, r))
    }
  }
  ...
  /**
   * Function: remove
   * Description: returns the set containg all elements except x.
   **/
  def remove(x: A): BST[A] = {
    this match{
      //If it's a leaf, return the current tree
      case Leaf() => this
      //If it's a nonempty tree
      case Node(e, l, r) =>
        //Remove element from the left subtree
        if (x < e){
          Node(e, l.remove(x), r)
        }
        //Remove element from the right subtree
        else if(x > e){
          Node(e, l, r.remove(x))
        }
        //The remove element is found, check if there's right subtree
        else {
          //Check if right subtree is leaf or nonempty tree
          r match{
            case Leaf() => l
            case Node(e, l1, r1) =>
              //Get the minimum value and the result of the tree
              val (m, res) = r.removeMin
              //Return the minimum value to be the current node with
              //left subtree, and result of the right subtree
              Node(m, l, res)
          }
        }
    }
...
object BST {
  ...
  /**
   * Function: build
   * Description: this functions converts the supplied list into
   * a BST[A] by recursively splitting the list up into sub-lists
   **/ 
  def build[A <% Ordered[A]](xs: List[A]): BST[A] = {
    if (xs.length == 0) {
      ...
      //The current will be the first element of the list
      val current = xs(0)
      //Filter so that eveything less than current
      val left = xs.filter((e:A) => e < current)
      //Filter so that eveything great than current
      val right = xs.filter((e:A) => e > current)
      Node(current, build(left), build(right))
    }
  }
}
